...
class TACTICPPO(PPO):
    ...
    def __init__(
        ...
        *args,
        ...
        stability_teacher_coef: float = 8.0,
        ...
        **kwargs,
        ...
        self.stability_teacher_coef = float(stability_teacher_coef)
        if self.stability_teacher_coef < 0.0:
            raise ValueError("stability_teacher_coef must be nonnegative")
        ...
        self._stability_teacher_backbone = None
        self._stability_teacher_head = None
    ...
    def set_training_stage(self, stage: str) -> None:
        ...
        if stage == "stability":
            ...
            self._stability_teacher_backbone = copy.deepcopy(
                self.policy.actor.physical_backbone
            ).eval()
            self._stability_teacher_head = copy.deepcopy(
                self.policy.actor.physical_head
            ).eval()
            for module in (
                self._stability_teacher_backbone,
                self._stability_teacher_head,
            ):
    ...
    def _stability_teacher_distillation_loss(self):
        """Keep a payload-adapted student close to the validated ZYB core."""

        if (
            self.training_stage != "stability"
            or self.stability_teacher_coef <= 0.0
            or self._stability_teacher_backbone is None
            or self._stability_teacher_head is None
        ):
            return None
        observations = self.storage.observations
        if not isinstance(observations, dict) or "policy" not in observations:
            return None
        policy_obs = observations["policy"].reshape(
            -1, observations["policy"].shape[-1]
        )
        actor_obs = self.policy.actor_obs_normalizer(policy_obs)
        physical_obs = actor_obs[:, : self.policy.actor.physical_core_obs_dim]
        with torch.no_grad():
            teacher_latent = self._stability_teacher_backbone(physical_obs)
            teacher_action = self._stability_teacher_head(teacher_latent)[:, :16]
        student_latent = self.policy.actor.physical_backbone(physical_obs)
        student_action = self.policy.actor.physical_head(student_latent)[:, :16]
    ...
    def update(self):
        ...
        if self.training_stage == "recovery":
            ...
            loss_dict = super().update()
        if self.training_stage == "stability":
            distillation_loss = self._stability_teacher_distillation_loss()
            if distillation_loss is not None:
                self.optimizer.zero_grad()
                (self.stability_teacher_coef * distillation_loss).backward()
                core_parameters = [
                    *self.policy.actor.physical_backbone.parameters(),
                    *self.policy.actor.physical_head.parameters(),
                ]
                torch.nn.utils.clip_grad_norm_(
                    core_parameters, self.max_grad_norm
                )
                self.optimizer.step()
                loss_dict["stability_teacher_loss"] = float(
