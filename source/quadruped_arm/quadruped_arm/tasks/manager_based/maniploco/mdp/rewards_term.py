...
def teacher_ensemble_match(env, sigma=0.20):
    """Match the student raw action to a gated mixture of safe teachers.

    The wheel dimensions are residual channels in the current action
    interface, so the wheel teacher target is zero while the bounded
    differential-drive action term supplies the actual command.
    """
    candidates = getattr(env, "teacher_ensemble_candidates", None)
    weights = getattr(env, "teacher_ensemble_weights", None)
    if candidates is None or weights is None:
        return torch.zeros(env.num_envs, device=env.device)
    action = env.action_manager.action
    if candidates.ndim != 3 or weights.ndim != 2:
        return torch.zeros(env.num_envs, device=env.device)
    if candidates.shape[0] != action.shape[0] or candidates.shape[-1] != action.shape[-1]:
        return torch.zeros(env.num_envs, device=env.device)
    error = (action.unsqueeze(1) - candidates).square().mean(dim=-1)
    likelihood = torch.exp(-error / max(float(sigma), 1.0e-4))
    return (weights * likelihood).sum(dim=1).clamp(0.0, 1.0)
...
def wheel_turn_support(
    ...
    # Some articulated wheel assemblies use the opposite body-yaw sign from
    # the textbook differential-drive convention.  Keep that convention
    # explicit at the reward boundary instead of silently changing the raw
    # wheel signs used by the actuator.
    wz_pred = float(wz_sign) * (v_right - v_left) / (track_width + 1e-6)
